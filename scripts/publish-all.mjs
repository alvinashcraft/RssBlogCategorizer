#!/usr/bin/env node
// Publishes the most recent rss-blog-categorizer-*.vsix to both the
...
function findLatestVsix() {
    const candidates = readdirSync(repoRoot)
        .filter(name => name.startsWith('rss-blog-categorizer-') && name.endsWith('.vsix'))
        .map(name => {
    ...
    if (candidates.length === 0) {
        console.error('No rss-blog-categorizer-*.vsix found in repo root. Run `npm run vsce:package` first.');
        process.exit(1);
    }
...
const vsceArgs = ['vsce', 'publish', '--packagePath', vsix];
